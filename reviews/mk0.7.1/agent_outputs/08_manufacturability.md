# Manufacturability Review — Homelab Modular Tower mk0.7.1
...
mk0.7.1 is a patch revision. The underlying CAD geometry, fastener counts, and assembly interfaces are materially unchanged from mk0.7. All three blockers from the mk0.7 manufacturability review persist unaddressed: the side panel mounting interface, the guide rail mounting interface, and the lack of quantitative BOM data. The design still carries **~121+ M3 screws**, **29+ heat-set inserts**, and **33 unique printable parts** — a fastener and insert burden that remains at the upper limit of hobbyist tolerance for a desktop tower.

The export manifest in mk0.7.1 no longer lists the duplicate `frame_bottom` / `bottom_structural_frame` and `frame_top` / `top_structural_frame` aliases (the unique STL count dropped from 35 to 33), but the underlying CAD functions still exist and the duplicate naming confusion in the codebase has not been resolved.

**ROD_LENGTH remains 321.5 mm**, which is geometrically inconsistent with the nut/washer seats in the frame and corner blocks. Real M5 threaded rods need ~340 mm minimum.

**Recommendation:** mk0.7.1 does not resolve the manufacturability blockers identified in mk0.7. Do not release for test build. The next revision (mk0.8 or equivalent) must define the two missing mounting interfaces, add BOM quantities, correct rod length, and provide assembly documentation before any hardware is ordered or printed.
...
A full recount of every M3 and M5 hole in `cad/config.py` and the part source files confirms the fastener burden is unchanged from mk0.7.

### M5 fasteners

| Component | Count | Notes |
|---|---|---|
| Threaded rods | 4 | `ROD_LENGTH = TOWER_HEIGHT = 321.5 mm` (see Threaded Rod Assembly) |
...
The corner blocks (`CORNER_BLOCK_SIZE = 24.0`, `CORNER_BLOCK_HEIGHT = 28.0`) each have an M5 nut seat on `<Z` and a washer seat on `>Z`. In the assembly, the corner blocks are at `z = TOWER_HEIGHT/2 = 160.75 mm`, between the bottom frame (`z = 0`) and top frame (`z = 321.5 mm`). The intended sandwich is: bottom nut → bottom frame → corner block → top frame → top nut. However, the corner-block nut/washer seats imply additional nuts may be intended at the corner block itself. If so, the M5 nut count would be 12 or 16. The design intent remains unclear — no assembly sequence document exists to clarify this.

### M3 fasteners

| Component | Count | Notes |
|---|---|---|
| Side panel tiles | 24 | 6 tiles × 4 holes each (`SIDE_PANEL_MOUNT_HOLE_DIAMETER = 3.2`) |
| Guide rail screws | 16 | 4 rails × 4 holes each (`METAL_RAIL_M3_SPACING = 70.0 mm`, `METAL_RAIL_HEIGHT = 287.5 mm`) |
| Tray device mounts | 24 | 6 trays × 4 holes (`TRAY_MOUNT_HOLE_OFFSET_X/Y`) |
...
Every `HEAT_SET_INSERT_M3_DIAMETER = 5.2 mm` / `HEAT_SET_INSERT_M3_DEPTH = 5.0 mm` reference in `cad/config.py` and the part source files is unchanged from mk0.7.
...
**Assessment:** 29 brass heat-set inserts remains a major manufacturability burden. Each insert requires:
...
Therefore, the actual rod length should be approximately **340 mm**, not 321.5 mm. The BOM still says "4x M5 threaded rods" without specifying length. The placeholder rod in the assembly (`create_m5_threaded_rod`) is exactly 321.5 mm and starts at `z = 0`, which means it does not protrude below the bottom frame or above the top frame. This is a geometric mismatch with reality that was not fixed in mk0.7.1.
...
All three problems identified in the mk0.7 review remain unaddressed in mk0.7.1.
...
Until this interface is defined, mk0.7.1 cannot be built.
...
From the export manifest (`part_dimensions.csv`), the printable plastic category contains 33 unique STL parts (down from 35 in mk0.7 because the duplicate `frame_bottom` / `bottom_structural_frame` and `frame_top` / `top_structural_frame` aliases were removed from the export list). However, the underlying CAD code still contains both `create_frame_bottom` / `create_frame_top` and `make_bottom_structural_frame` / `make_top_structural_frame`, so the duplicate naming confusion in the codebase has not been resolved — only the export manifest was cleaned up.

| # | Part | X (mm) | Y (mm) | Z (mm) | Notes |
|---|---|---|---|---|---|
| 1 | bottom_fan_cartridge | 142 | 156 | 12 |  |
...
| 8 | external_ssd_bay | 172 | 179 | 32 |  |
| 9 | foot_socket | 42 | 42 | 3 | 4× needed |
| 10 | front_stability_wing | 250 | 47 | 10 |  |
| 11 | left_foot_extension | 42 | 190 | 10 |  |
| 12 | left_side_panel_lower | 176 | 13 | 100.9 |  |
| 13 | left_side_panel_middle | 176 | 5.2 | 100.9 |  |
| 14 | left_side_panel_upper | 176 | 13 | 100.9 |  |
| 15 | mikrotik_tray | 172 | 179 | 32 |  |
| 16 | mini_pc_airflow_duct | 88 | 134 | 62 |  |
| 17 | mini_pc_tray | 172 | 179 | 32 |  |
| 18 | mini_pc_tray_stop | 18 | 6 | 22 |  |
| 19 | power_bus_cover | 46 | 5 | 266 |  |
| 20 | power_bus_panel | 34 | 8 | 276 |  |
| 21 | raspberry_pi_tray | 172 | 179 | 32 |  |
| 22 | rear_service_spine | 80 | 40 | 298 |  |
| 23 | rear_service_spine_cover | 46 | 3 | 296 |  |
| 24 | rear_stability_wing | 250 | 47 | 10 |  |
| 25 | right_foot_extension | 42 | 190 | 10 |  |
| 26 | right_side_panel_lower | 176 | 13 | 100.9 |  |
| 27 | right_side_panel_middle | 176 | 5.2 | 100.9 |  |
| 28 | right_side_panel_upper | 176 | 13 | 100.9 |  |
| 29 | ssd_expansion_tray | 172 | 179 | 32 |  |
| 30 | top_fan_grille | 190 | 190 | 4 |  |
| 31 | top_structural_frame | 190 | 190 | 13 |  |
| 32 | ups_power_tray | 187 | 179 | 32 |  |
| 33 | foot | 34 | 34 | 32 | 4× TPU |

**Notes:**
- The user must also source: 4× M5 rods, 4× metal guide rails (10×3 mm, 287.5 mm), 2× 120 mm fans, and ~120+ screws.
- When counting printed instances (including 4× corner_block, 4× foot_socket, 4× foot), the total printed instance count is ~40+.

**Assessment:** 33 unique printed parts plus hardware is still a large project. For a first test build, consider printing only a subset (e.g., one frame, one corner block, one tray, one side panel) to validate fits before committing to the full print.
...
|---|---|---|---|
| Module trays (all 6) | Yes — handle pocket | `TRAY_FRONT_HANDLE_WIDTH = 64.0` handle pocket is a 64×4.2×13 mm cavity in the front plate. The top surface of the pocket is a 64 mm bridge. | Hard — PETG supports fuse to the part |
...
PETG is notoriously difficult for support removal. It bonds aggressively to itself. The tray handle pocket will have support material fused to the 13 mm deep pocket walls. Removing it without damaging the front plate is hard. The `PRINTING.md` says "Use supports only for local openings if slicer preview shows poor bridges. Prefer adding chamfers and simple rectangular clearances in v2 over relying on heavy supports." This is good advice, but mk0.7.1 still has the handle pocket without chamfers or bridge-breaking features.
...
|---|---|---|
| Module trays | Base flat on bed | 172×179 mm footprint; handle pocket needs supports if base is flat. If printed vertically, the tray rails would need supports. |
...
| Rear service spine | Back or side | 80×298 mm or 40×298 mm footprint. Tall, thin wall. |
...
`tower_assembly.py` is a geometric placement script, not a build instruction. It instantiates every part at once. There is no assembly sequence document, no build order, and no guidance for the user. `docs/ASSEMBLY.md` does not exist.
...
4. **When do you install the rear service spine?** The spine is 298 mm tall and sits behind the modules. It likely needs to go in before the trays, but its cover mounts may interfere with tray rails.
...
The following issues must be resolved before any test print or build. **All three blockers from mk0.7 remain unaddressed in mk0.7.1.**
...
### High priority (fix before next revision)
...
9. **Increase middle side panel thickness** from 5.2 mm to at least 8 mm, or add a brim recommendation in `PRINTING.md`.
...
12. **Consider printing a single module subset first** (one frame, one corner block, one tray, one side panel) to validate clearances before printing all 33+ parts.
13. **Remove duplicate frame functions** from `cad/parts/frame.py` (`create_frame_bottom` / `make_bottom_structural_frame` and `create_frame_top` / `make_top_structural_frame` are redundant aliases).
