...
The mk0.7.1 power system and cable management design is **conceptually sound** but **not physically viable** in its current state. The power bus panel and cover remain structural placeholders that will not survive real assembly. The rear service spine has insufficient cable capacity for the declared device count. Connector zones lack the clearance required for real panel-mount connectors. Strain relief is undersized. The DC UPS placeholder volume is optimistic. Most critically, there is no defined cable path for the bottom fan, and the 220 V external PSU is not physically integrated.

**The only change in mk0.7.1** is the power bus cover screw hole alignment: holes are now at `±POWER_BUS_PAD_SCREW_OFFSET_X` (±7 mm), matching the panel pad holes. This fix is correct but cosmetic; it does not address the fundamental rigidity, capacity, or routing blockers.
...
### Geometry
- Panel dimensions: **34 mm wide × 7.5 mm thick (max) × 275.5 mm tall** (from `config.py`: `POWER_BUS_WIDTH = 34.0`, `POWER_BUS_THICKNESS = 3.0`, `POWER_BUS_HEIGHT = 275.5`).
...
### Evidence
- `config.py:531–533`: `POWER_BUS_THICKNESS = 3.0`, `POWER_BUS_HEIGHT = TOWER_HEIGHT - 46.0 = 275.5`.
...
### Geometry
- Cover dimensions: **46 mm wide × 5.0 mm thick (max) × 265.5 mm tall** (from `config.py`: `POWER_BUS_COVER_WIDTH = 46.0`, `POWER_BUS_COVER_THICKNESS = 2.4`, `POWER_BUS_COVER_HEIGHT = 265.5`).
...
The cover is now mounted with M3 screws at the correct `±7 mm` X offsets, matching the panel rail pads. The code uses `for _, z in cfg.POWER_BUS_RAIL_LABELS: cover.hole(M3_CLEARANCE)` at the rail Z positions, with holes at `(-7, z)` and `(7, z)`. This means there are still only **4 screw pairs** (8 holes) along the entire 265 mm height, spaced at rail positions (70, 25, -20, -65). For a 265 mm tall cover, 4 screw pairs are insufficient to prevent bowing between screws. The span between screws is ~45–90 mm, and a 2.4 mm PETG panel will sag between mounts.

### Fix Verification
The mk0.7.1 change in `service_spine.py:57–60` correctly sets the cover holes to `±POWER_BUS_PAD_SCREW_OFFSET_X` (7.0 mm), matching the panel M3 holes. This is a valid bug fix, but it does not change the structural assessment.

### Evidence
- `config.py:551–554`: `POWER_BUS_COVER_THICKNESS = 2.4`, `POWER_BUS_COVER_HEIGHT = 265.5`.
- `service_spine.py:44–61`: Cover is a `box(46, 2.4, 265.5)` with guard rails and 8 M3 holes at ±7 mm.
- `part_dimensions.csv:82`: `power_bus_cover` bounding box is `46.0 × 5.0 × 265.5` mm.

### Grade: **CONFIRMED** — The cover will flex significantly and is insufficiently supported. The hole alignment fix is correct but insufficient for structural viability.
...
- `KNOWN_LIMITATIONS.md:7`: Explicitly states fan cable routing is not detailed.
...
This is a placeholder. However, for an engineering review, the absence of fuse geometry is a design gap. The fuse block zone in the UPS tray overlaps with the DC-DC zone at (-6, 58, 38, 24) in the range y=58–78, confirming the component zones are not a real layout.
...
The `KNOWN_LIMITATIONS.md` explicitly acknowledges this: "Реальный routing провода вентилятора в Rear Service Spine пока не детализирован." (Real fan cable routing to Rear Service Spine is not yet detailed.)
...
- `KNOWN_LIMITATIONS.md:7`: Explicit known issue about fan cable routing.
